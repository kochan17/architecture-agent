...
description: Use when creating or reviewing software architecture diagrams and documentation with Codex-controlled JSON for Architecture Agent. Covers common architecture foundations, C4/arc42/ISO 42010-style concerns, cloud providers, databases, quality gates, decisions, risks, and evidence-backed review output.
---

# Architecture Agent

Use this skill when the user asks Codex to create, improve, or review an architecture diagram for Architecture Agent without calling a generation API.
...
4. Include architecture metadata, not only nodes and edges:
   ...
   - `meta.reviewFindings`
   - `meta.sources`
5. Run `node skills/architecture-agent/scripts/validate_architecture_json.mjs outputs/codex-diagram.json`.
...
The diagram JSON must remain compatible with Architecture Agent:
